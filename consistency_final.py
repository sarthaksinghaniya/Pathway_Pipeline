"""
Backstory vs Evidence Consistency Classifier - Final Version
...
def _get_global_classifier(model_client=None):
    """Get or create global classifier instance."""
    global _global_classifier
    if _global_classifier is None or model_client is not None:
        _global_classifier = ConsistencyClassifier(model_client=model_client, contradiction_threshold=0.7)
...
def predict_consistency(backstory: str, retrieved_chunks: List[str]) -> int:
    ...
    This is the required function signature from the specification.
    ...
    # Evaluate all chunks
    evaluations = classifier.evaluate_all_chunks(backstory, retrieved_chunks)
    
    # Apply deterministic aggregation
    final_decision = classifier.aggregate_decisions(evaluations)
...
def test_consistency_classifier():
    ...
    decision = predict_consistency(backstory, chunks)
    ...
    classifier_llm = _get_global_classifier(model_client=mock_client)
    
    decision_llm = classifier_llm.aggregate_decisions(classifier_llm.evaluate_all_chunks(backstory, chunks))
