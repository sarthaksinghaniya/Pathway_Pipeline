"""
Semantic retrieval module for finding relevant novel chunks based on backstories.
"""
...
class BackstoryRetriever:
    """Handles retrieval of relevant novel chunks for given backstories."""
    
    def __init__(self, vector_store: SemanticVectorStore, top_k: int = 10, similarity_threshold: float = 0.3):
        """
        Initialize the retriever.
        
        Args:
            vector_store: Initialized semantic vector store
    ...
    def set_indexed_table(self, indexed_table):
        """Set the indexed table for retrieval."""
    ...
    def retrieve_for_backstory(self, backstory: str, novel_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if self.indexed_table is None:
            raise ValueError("Indexed table not set. Call set_indexed_table() first.")
